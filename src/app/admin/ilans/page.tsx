...
export default function AdminIlanlarPage() {
  ...
  const calculateStats = (ilanlar: Ilan[]) => {
    const stats = {
      ...
      onayBekleyen: ilanlar.filter(i => !i.isApproved).length,
  ...
  const handleApprovalChange = async (ilanId: string, approved: boolean) => {
    try {
      ...
      console.error('Onay durumu güncellenemedi:', error);
    }
  };
  ...
  const getIlanDurumuBadge = (durum: string) => {
    const variants = {
      ...
      tamamlandi: { label: 'Tamamlandı', color: 'bg-blue-100 text-blue-800', icon: CheckCircle2 },
  ...
  const filteredIlanlar = ilanlar.filter(ilan => {
    const matchesSearch = ilan.baslik.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         ilan.user_id.email.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = statusFilter === 'all' || ilan.ilan_durumu === statusFilter;
    const matchesApproval = approvalFilter === 'all' || 
                           (approvalFilter === 'approved' && ilan.isApproved) ||
                           (approvalFilter === 'pending' && !ilan.isApproved);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Tüm ilanları yönetin ve onaylayın</p>
        </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.onayBekleyen}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <Card key={ilan._id} className="hover:shadow-lg transition-shadow">
                <div className="relative">
                  ...
                  <div className="absolute top-2 left-2 flex gap-2 flex-wrap">
                    ...
                    {getApprovalBadge(ilan.isApproved)}
                  </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => window.open(`/ilans/${ilan._id}`, '_blank')}
                          >
                            <Eye className="h-3 w-3 mr-1" />
                            Görüntüle
                          </Button>
                        ...
                        <div className="flex gap-1">
                          {!ilan.isApproved ? (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleApprovalChange(ilan._id, true)}
                            >
                              <CheckCircle className="h-3 w-3 mr-1" />
                              Onayla
                            </Button>
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleApprovalChange(ilan._id, false)}
                            >
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {searchTerm || statusFilter !== 'all' || approvalFilter !== 'all' ? 'İlan Bulunamadı' : 'Henüz İlan Yok'}
              </h3>
